...
export default async function PublicAppShell({
  ...
  const optimizeHeaderLogo = isSupabaseStorageUrl(headerLogoUrl);
  const isNativeApp = await isNativeAppRequest();

  const shellClassName = isNativeApp
    ? "h-[100svh] h-[100dvh] overflow-hidden bg-[linear-gradient(180deg,#f3e4d9_0%,#efe5dc_26%,#f7f2ec_64%,#f9f7f3_100%)] px-0 pb-0 pt-[env(safe-area-inset-top,0px)]"
  ...
  const headerClassName = isNativeApp
    ? "shrink-0 border-b border-stone-200/75 bg-white/80 px-4 pb-4 pt-4 backdrop-blur-sm"
    : "shrink-0 border-b border-stone-200/80 px-5 pb-4 pt-5";
  const contentClassName = isNativeApp
    ? "min-h-0 flex-1 overflow-y-auto px-4 pb-[calc(var(--app-bottom-nav-height)+1rem)] pt-4"
          ...
          <div className="flex items-center gap-3">
            {headerLogoUrl ? (
              <Image
                ...
                height={48}
                sizes="48px"
                unoptimized={!optimizeHeaderLogo}
